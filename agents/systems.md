...
Для логики:
- **желательный максимум** — failing test до фикса (тест проваливается, потом фикс, потом тест проходит);
- **минимум, который должен быть всегда** — воспроизводимый сценарий текстом: что вводим, что нажимаем, что должно получиться.

Для интеграций: сначала проверка входа/выхода на границе компонента.

Если на проекте тесты ещё не настроены — это технический долг, зафиксируй в `clients/<name>/project-log.md` и постепенно закрывай.
