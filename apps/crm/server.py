...
PORT = int(os.getenv("CRM_PORT", "3005"))
DB_PATH = Path(os.getenv("CRM_DB_PATH", str(BASE_DIR / "data" / "crm.sqlite")))
ADMIN_USER = os.getenv("CRM_ADMIN_USER", "inna")
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == "/health":
            self.send_json(200, {"ok": True, "service": "crm"})
            return
    ...
    def require_auth(self):
        ...
        self.send_header("WWW-Authenticate", 'Basic realm="Inna CRM"')
        self.end_headers()
        return False
...
<head>
  ...
  <title>Моя база клиентов</title>
  <style>
    body {{ margin:0; font-family: system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif; background:#f7f3ec; color:#102006; }}
    header {{ padding:24px; background:#103206; color:#fff2e9; }}
    main {{ max-width:1080px; margin:0 auto; padding:24px; }}
    h1 {{ margin:0; font-size:28px; }}
    .lead {{ background:white; border:1px solid #e5dccf; border-radius:8px; padding:18px; margin:0 0 14px; }}
    .lead-head {{ display:flex; justify-content:space-between; gap:16px; align-items:center; }}
    .lead h2 {{ margin:0; font-size:20px; }}
    .lead-head span {{ background:#fff2c7; color:#103206; padding:4px 8px; border-radius:999px; font-size:13px; }}
    .result {{ border-left:3px solid #d4af37; padding-left:12px; margin:12px 0; color:#34462b; }}
    select, button {{ min-height:36px; border-radius:6px; border:1px solid #cfc4b6; padding:6px 10px; }}
    button {{ background:#103206; color:#fff2e9; cursor:pointer; }}
  </style>
</head>
<body>
  <header><h1>Моя база клиентов</h1><p>Последние 200 лидов</p></header>
...
def main():
    ...
    print(f"CRM MVP listening on http://{HOST}:{PORT}")
    httpd.serve_forever()
