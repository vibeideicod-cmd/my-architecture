...
- [ ] В `brief.md` заполнено поле «Что запомнится» — и эта деталь действительно реализована в вёрстке
...
- [ ] Цветовая схема НЕ «фиолетовый градиент на белом» и не другие штампы AI-генерации
- [ ] Раскладка НЕ предсказуемая шаблонная (3 одинаковые карточки в ряд + центральный CTA = AI slop)
